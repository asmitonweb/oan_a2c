...
def execute():
    ...
    frappe.db.sql(
        "DELETE FROM `tabA2C Farmer Profile` WHERE lead_id='POSTMAN_TEST_LEAD'"
    )
    ...
    frappe.db.sql(
        "UPDATE `tabA2C Lead` SET name='POSTMAN_TEST_LEAD' WHERE name=%s", lead.name
    )
    frappe.db.commit()
    print(f"  ✔ Created Lead: POSTMAN_TEST_LEAD")
    ...
    print(f"  ✔ PASS — Loan status updated to Processing")
    ...
    res2 = update_loan_status(
        application_id=app_id, status="Approved", reason=reason
    )
    ...
    print(f"  ✔ PASS — Loan status updated to Approved")
    ...
    audit_count = frappe.db.count(
        "A2C Loan Application Audit Event", {"loan_application": app_id}
    )
    assert audit_count == 2, f"Expected still 2 audit events, got {audit_count}"
    print(f"  ✔ PASS — No audit event created for failed transition (still 2 events)")
    ...
    frappe.db.sql(
        "UPDATE `tabA2C Loan Application` SET docstatus=0 WHERE name=%s", app_id  # bank-scope-exempt: test cleanup
    )
